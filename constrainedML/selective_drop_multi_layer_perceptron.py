...
class SelectiveDropMultilayerPerceptron(BaseConstrainedMultilayerPerceptron):
    def __init__(
        self,
    ...
    def fit(self, X, y, min_coef=None, max_coef=None):
        ...
        self.min_coef_ = self._verify_coef(
            original_feature_count, min_coef, -np.inf
        ).flatten()
        self.max_coef_ = self._verify_coef(
            original_feature_count, max_coef, np.inf
        ).flatten()
        self.selected_features = self._find_non_zero_indices(
            self.min_coef_, self.max_coef_
        )
        ...
        try:
            for it in range(self.max_iter):
                ...
                for batch_slice in gen_batches(n_samples, batch_size):
                    ...
                    self._update_coef_using_constrain(self.coefs_)
    ...
    def _update_coef_using_constrain(self, coef_grads):
        for coef_idx, (min_coef, max_coef) in enumerate(
            zip(
                self.min_coef_[self.selected_features],
                self.max_coef_[self.selected_features],
            )
        ):
    ...
    def _find_non_zero_indices(self, min_coef, max_coef):
